...
describe("TelegatorDataStack", () => {
  test("declares the three tables: §7.2 L633's two, plus targets (R58)", () => {
    templateFor().resourceCountIs("AWS::DynamoDB::Table", 3);
  });

  test("names them with the §9.2 L900 environment prefix", () => {
  ...
  describe("sources", () => {
    /** §7.2 L637 — `status-index`: PK `status`, and no sort key. */
    ...
    /**
     * §3.1 L199-231 reads or writes nearly every attribute of a selected source
  ...
  describe("messages", () => {
    ...
    test("has both indexes §7.2 L638 names", () => {
      expect(
        gsisOf(templateFor())
          ?.map((g) => g.IndexName)
          .sort(),
    ...
    /**
     * §7.2 L640 — status-index "uses INCLUDE with dashboard-visible attributes
     ...
     * §8.3 L802's expandable member list must be a lazy base-table read.
     */
    ...
    test("status-index projects what §8.3 L802 and §8.5 L836 render (R27)", () => {
     ...
     * is accepted by `cdk diff` and refused by DynamoDB (§7.2 L642), so this is
    ...
    /**
     * R44 — §7.2 L640 called this "the one query that needs vectors". There are
    ...
    /** §9.1 L887 and §10.4 L1032 — the only §10.4 row verifiable without a deployment. */
