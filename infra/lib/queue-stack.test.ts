...
describe("TelegatorQueueStack", () => {
  /** §7.3 L648-654 — three queues, and "Each has a matching DLQ". */
  ...
  /**
   * §7.3 L654 — "Message retention: 14 days (the SQS maximum) on every queue
  ...
  /** §7.3 L666 — visibility is 6x the 300 s function timeout. */
  test.each([
    "telegator-dev-analyze",
    "telegator-dev-aggregate.fifo",
    "telegator-dev-publish.fifo",
  ...
  describe("queue types (§7.3 L650-652)", () => {
    ...
    test.each(["telegator-dev-aggregate.fifo", "telegator-dev-publish.fifo"])(
      "%s is FIFO",
      /**
       * AC-3.9 (§3.3 L310) and AC-4.6 (§3.4 L363) both rest on this. A FIFO queue
    ...
    /**
     * The producers always supply an explicit MessageDeduplicationId (§3.2 L258,
     * §3.3 L295), so content-based deduplication must stay off — enabling it
  ...
  describe("redrive policies (§7.3 L650-652)", () => {
    test.each([
  ...
  describe("the settle delay (§3.3 L296, §7.3 L652, R19)", () => {
    /**
     * R19: SQS FIFO supports only a queue-level DelaySeconds, so §3.3 L296's
    ...
    test("§11.4 L1047 calls 300 s a starting value, so an override reaches the queue", () => {
  ...
  test("names carry the §9.2 L900 environment prefix", () => {
