...
describe("TelegatorQueueStack", () => {
  /** §7.3 L644-650 — three queues, and "Each has a matching DLQ". */
  ...
  /**
   * §7.3 L650 — "Message retention: 14 days (the SQS maximum) on every queue
  ...
  /** §7.3 L662 — visibility is 6x the 300 s function timeout. */
  test.each([
    "telegator-dev-analyze",
    "telegator-dev-aggregate.fifo",
    "telegator-dev-publish.fifo",
  ...
  describe("queue types (§7.3 L646-648)", () => {
    ...
    test.each(["telegator-dev-aggregate.fifo", "telegator-dev-publish.fifo"])(
      "%s is FIFO",
      /**
       * AC-3.9 (§3.3 L306) and AC-4.6 (§3.4 L359) both rest on this. A FIFO queue
    ...
    /**
     * The producers always supply an explicit MessageDeduplicationId (§3.2 L254,
     * §3.3 L291), so content-based deduplication must stay off — enabling it
  ...
  describe("redrive policies (§7.3 L646-648)", () => {
    test.each([
  ...
  describe("the settle delay (§3.3 L292, §7.3 L648, R19)", () => {
    /**
    ...
    test("§11.4 L1043 calls 300 s a starting value, so an override reaches the queue", () => {
  ...
  test("names carry the §9.2 L896 environment prefix", () => {
