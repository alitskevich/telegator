...
/** R37 — the three fields §8.4 L808 will accept for a message. */
...
export interface MessagesTableProps {
  ...
  /** §8.4 L810 — `editor`, and soft: the record survives, R16 hides it. */
...
export function MessagesTable(props: MessagesTableProps) {
  ...
  return (
    <>
      ...
      {/* §8.2 L775 — the tab is `?status=`, so each is a link and the current
        ...
        <tr className="member-panel">
          <td colSpan={columnCount}>
            {members === undefined ? (
              ...
              // §6 L584's create branch writes one member, so an empty map means
