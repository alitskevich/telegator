...
describe("the captured markup", () => {
  ...
  test("uses the exact chunk marker §3.1 L209 splits on", () => {
    expect(CHUNK_MARKER).toBe('<div class="tgme_widget_message_wrap js-widget_message_wrap">');
  });

  test("puts page chrome before the first marker, which §3.1 L209 discards", () => {
  ...
  test("carries the six HTML entities §3.1 L216 decodes", () => {
    const html = telegramFixture("multiPost");
   ...
   * an emoji can precede the photo. §3.1 L218 says to take the *first*
