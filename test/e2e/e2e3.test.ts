...
/**
 * E2E-3 (§10.2 L1000) — "Re-running the scraper with no new upstream content
 ...
 * poll (§7.5 L693's 30-minute rule), not a second call in the same instant: at
...
describe("E2E-3 (§10.2 L1000)", () => {
  ...
  /** §4.1 L382 — a source yielding nothing repeatedly has to become observable. */
 ...
 *
 * §3.1 L223 offers the `members` map as a safety net if the cursor ever fails.
 * It holds only while the re-scraped duplicate lands on the same `date`, because
 * §6 L564 looks for candidates in `date-index` for that date alone. The two
 ...
 */
describe("E2E-3 — R29's bound on §3.1 L223's safety net", () => {
  ...
  test("across midnight the duplicate is refused rather than published again", async () => {
    ...
    clock.advance(NEXT_POLL_MS);
    await sources.patch(SOURCE, { lastItemId: undefined });
    // §6 L545 makes the UTC date a correctness rule, and `date-index` is
