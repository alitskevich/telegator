...
/**
 * E2E-5 (§10.2 L1002) — "**Replaying the entire aggregate DLQ leaves the messages
 ...
 * than it was. §6 L597 stamps `ts: now()` on every write, so a replay under a
 * real clock cannot leave that byte unchanged. §6 L597's second concession is
...
async function runThenReplay() {
  ...
  // §3.5 L368 — the replay puts the same bodies back on the source queue.
...
describe("E2E-5 (§10.2 L1002)", () => {
   ...
   * that, every replay would rewrite every member block and §3.4 L323's ordering
  ...
  /**
   * Item 8.6 found a third deviation here — §6 L585's merge branch wrote
   * `status: "topublish"` unconditionally, so a replay returned every published
   * message to the publish queue and §3.4 L349 edited the live post with
   ...
   * spec's own: §6 L597's `ts` stamp and §6 L597's centroid drift.
   */
  ...
  /**
   * §3.4 L354's publish result survives. A replay that cleared `tgId` would turn
  ...
  /** §6 L588's create branch is conditional (R38); a replay must not reach it. */
  ...
  /**
   * R45 — `unionMatchKeys` is idempotent, so the field §6 L597 conceded would
