...
/**
 * §8.5 L835's category chart spans two modules that never call each other: the
...
describe("the analyze log line is what the category query reads", () => {
   ...
   * stage really produced, and check the counts §8.5 L835's chart would show.
   */
  ...
  /**
   * §7.7 L741 refuses a per-category CloudWatch metric — "Thirty-five category
   ...
   * one of enqueued items only, understating exactly the categories §5.2 L459
