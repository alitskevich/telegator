 ...
 *
 * Pure: it writes no table row and enqueues nothing (§6 L590–591), which is what
 ...
 *  - **R9** — `date-index` projects no `members` (§7.2 L640), so a merge is
 ...
 *    because §3.3 L287 lets later members overwrite what a replay would match on.
 */
...
export interface DedupDeps {
  ...
  /**
   * §10.3 L1021 requires recalibration before production. Injected so that
...
export interface DedupResult {
  ...
  /** §6 L591 — every touched message id. */
  readonly toPublish: readonly string[];
  /** §7.2 L644 — alarmed above 500, where §6's in-memory assumption stops holding. */
...
interface Pending {
   ...
   * — a no-op", but §6 L585's merge branch also writes `status: "topublish"`,
   ...
   *
   * A snapshot rather than a running flag, because §3.3 L287 has each item's
...
export async function dedupBatch(
  ...
  // All items in a batch share one date (§7.3 L651's FIFO group) and nothing is
  // written until the caller applies the result, so §6 L564's per-item query
   ...
   * `bandFieldsOf` needs the descriptive fields §7.2 L640 does not project, so
  ...
  /**
   * §6 L585 — the create/merge tail, shared by the items decided on sight and
  ...
  const absorb = async (item: AnalyzedItem, key: MatchKey, matchId: string | undefined) => {
    ...
    if (state !== undefined) {
      // §3.3 L283 — the cap rejects only a *new* key, so a replay of an item
    ...
    // §6 L585–597, with R11's preservation.
    ...
    const next: Pending =
      state === undefined
        ...
        : {
            ...state,
            // R45 — the union, where §3.3 L284 took the elementwise mean.
            key: unionMatchKeys(state.key, key),
            members: { ...state.members, [item.id]: block },
            addedMembers: { ...state.addedMembers, [item.id]: block },
            // §3.3 L286 — item side first, so a replay is a fixed point.
            tags: mergeTags(item.tags, state.tags),
            // §3.3 L285 uses `??`, which keeps an empty-string image (R30).
            image: state.image ?? item.image,
            // §3.3 L287 — the newest item's descriptive fields overwrite.
            title: item.title,
            category: item.category,
            country: item.country,
            location: item.location,
  ...
  for (const { item, key } of keyed) {
     ...
     * that message by identity, whatever it would now score: §3.3 L287 lets
    ...
    const better = (candidate: Best) => {
      // §6 L577 records any improvement, with no threshold test.
      if (best === undefined || candidate.score > best.score) best = candidate;
    };

    // Pass 1 — messages touched earlier in this batch (§6 L572–550).
    ...
    /**
     * Pass 2 — stored messages on the same date (§6 L576).
     *
  ...
  // again. Filtered here rather than per item because §3.3 L287's overwrite
 ...
 *
 * §10.3 L1011 ranks a false merge as the costlier error, so a throw, a timeout or
 ...
 *
 * §7.2 L640's projection carries no `title`, `category` or `location`, and
 ...
 *
 * One read per band pair, at most ten per batch (§7.3 L651), and only here:
...
function toWrite(state: Pending, ts: number): DedupWrite {
  ...
  const shared = {
    memberCount,
    /**
     * R45 — the three projections §7.2 L640 scores on, written from the union
