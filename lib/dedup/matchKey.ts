...
/**
 * The deduplication match key (R46), replacing §6 L518-518's embedding.
 *
 * Built only from the fields §5.2 L451-461 makes the classifier emit in English
...
/** `peoples` and `properNames` are comma-separated by §5.2 L459-460. */
function splitCommas(value: string | undefined): string[] {
  return value === undefined ? [] : value.split(",").map(canonical);
}

/** `title` is "three words, English" (§5.2 L451), so whitespace is the separator. */
...
/**
 * R44 — §7.2 L640–640 stores a dedup match as three projections for the
