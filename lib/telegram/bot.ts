...
/** §4.2 L386 — "Base: `https://api.telegram.org/bot{token}`". */
export const TELEGRAM_API_BASE = "https://api.telegram.org";

/**
 * §3.4 L350 — "`parse_mode: html`". The spec writes it lowercase; the Bot API
 * constant is `HTML`, and an unrecognised value is rejected outright.
 */
export const DEFAULT_PARSE_MODE = "HTML";

/**
 * §3.4 L352 — "≥3 s pause after each send".
 *
 * The rule lives in §3.4, not §3.5: §4.2 L393 says "Pacing and retry are
 * specified in §3.5", which is wrong — §3.5 is the DLQ replay handler.
 */
export const SEND_PAUSE_MS = 3_000;

/** §3.4 L352 — "one retry on `429` honouring `parameters.retry_after`". */
export const TOO_MANY_REQUESTS_STATUS = 429;

const MS_PER_SECOND = 1_000;

/**
 * §3.4 L352 names no delay for a 429 that omits `parameters.retry_after`, so
 * this module picks one: a full minute, because §4.2 L391 puts the ceiling at
 ...
 * 300 s timeout with a batch size of 1 (§3.4 L323), so a minute fits.
 */
export const FALLBACK_RETRY_AFTER_MS = 60_000;

/** §3.4 L352 — one retry, so at most two attempts. */
const MAX_ATTEMPTS = 2;

export interface HttpPostResponse {
  /**
   * Kept only for the 429 retry decision (§3.4 L352). It is deliberately *not*
   * the error signal — see `isOk` below and §4.2 L390.
   */
...
export interface TelegramBotOptions {
  ...
  /**
   * §7.6 L705 keeps `telegator/telegram-bot-token` in Secrets Manager. The
...
/**
 * Narrows an unknown body to the §4.2 L390 envelope, or `undefined` when it is
...
export function createTelegramBot(options: TelegramBotOptions): TelegramBot {
   ...
   * with HTTP 429, but §4.2 L390 warns that failures can arrive as 200s, so an
  ...
  const fail = (method: Method, description: string): Envelope => {
    // §7.7 L738 — `TelegramApiErrors`, dimensioned by `Method`. Counted here on
    ...
    // ~20 msg/min ceiling (§4.2 L391) is being hit.
  ...
  const call = async (
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      ...
      // §3.4 L352 — "≥3 s pause after each send". The pause is unconditional,
      ...
      // §4.2 L390 — `ok` is the error signal, not the status code. Reading the
    ...
    // Both attempts were rate limited: §3.4 L352 allows exactly one retry, so
    // the second 429 is the caller's failure to handle (publish throws, SQS
    // retries, and the message ultimately DLQs — §3.4 L354, AC-4.7).
    return last;
  };

  /**
   * §4.2 L388 goes through `chatIdFor`, which is idempotent, so a caller may
