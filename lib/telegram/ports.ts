 ...
 * Bot API sink (§4.2).
 *
 * Both sit behind interfaces so no test touches the network. §4.1 L376 calls
...
/** §4.2 L387 — 4096 characters per message. */
export const TELEGRAM_MESSAGE_LIMIT = 4096;

/** §4.2 L387 — 1024 characters per photo caption. */
export const TELEGRAM_CAPTION_LIMIT = 1024;

/** §4.2 L384 — "Chat id is the target channel with a leading `@`." */
...
export interface HttpFetcher {
  /**
   * §3.1 L207 — "Non-2xx yields an empty string, not an exception."
   *
   * That is load-bearing rather than lenient: §3.1 L220 turns an empty fetch
   * into a `zeroYieldRuns` increment, which is the only way §4.1 L378's
...
/**
 * The Bot API response envelope (§4.2 L386).
 *
...
export interface TelegramResponse {
  ...
  /** Present on a 429; §3.4 L348 honours `retry_after` for its single retry. */
