...
describe("fakeFetcher", () => {
  ...
  /**
   * §3.1 L207 — "Non-2xx yields an empty string, not an exception." That is not
   * a convenience: §3.1 L221 turns an empty fetch into a zeroYieldRuns
   * increment, which is how §4.1 L382's staleness alarm ever fires. A thrown
...
describe("fakeBot", () => {
  ...
  /**
   * §4.2 L390 — "Failures return HTTP 200 with `{ok: false, description}` —
