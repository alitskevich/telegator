...
export interface FetchResponse {
  /** True for a 2xx. §3.1 L207's "non-2xx" test, without a status constant. */
  readonly ok: boolean;
    ...
    get: async (url, headers) => {
      try {
        const response = await doFetch(url, {
          ...
          // `?after=` cursor already appended (§3.1 L207) — and is used
          // verbatim; re-encoding it here could only corrupt the cursor.
          method: "GET",
          // §3.1 L207's browser-like headers are the caller's: `t.me/s/` serves
        ...
        // §3.1 L207 — non-2xx is an empty string. Reading the body of an error
