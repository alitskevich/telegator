 ...
 *
 *  - `HttpFetcher` (§3.1 L205) — non-2xx yields an empty string, not an
 *    exception. Load-bearing rather than lenient: §3.1 L218 turns an empty fetch
 *    into a `zeroYieldRuns` increment, the only thing that makes §4.1 L376's
 ...
 *
 *  - `HttpPost` (§4.2) — the status still matters, because §3.4 L346 retries on
 *    a `429`. It hands back status and body and lets `bot.ts` decide, whose
 *    error signal is `ok` rather than the status (§4.2 L384).
 *
...
/**
 * §7.5 L687 gives `scrape` a 300 s budget and §3.1 L203 spends it on up to ten
...
/**
 * §7.5 L689 gives `publish` 300 s for a batch of one (§3.4 L311). Two attempts
 * at 30 s plus `bot.ts`'s 60 s fallback `retry_after` and its 3 s pause
 * (§3.4 L346) comes to 123 s, comfortably inside that.
 */
export const BOT_API_TIMEOUT_MS = 30_000;

/** The slice of the `Response` interface these adapters use. */
export interface FetchResponse {
  /** True for a 2xx. §3.1 L205's "non-2xx" test, without a status constant. */
  readonly ok: boolean;
    ...
    get: async (url, headers) => {
      try {
        const response = await doFetch(url, {
          ...
          // `?after=` cursor already appended (§3.1 L205) — and is used
          // verbatim; re-encoding it here could only corrupt the cursor.
          method: "GET",
          // §3.1 L205's browser-like headers are the caller's: `t.me/s/` serves
        ...
        // §3.1 L205 — non-2xx is an empty string. Reading the body of an error
        ...
        // yielded nothing", which §3.1 L218 counts and §4.1 L376 alarms on.
 ...
 * `bot.ts` rejects any body that is not the §4.2 L384 envelope, reporting the
...
export function createHttpPost(options: HttpOptions = {}): HttpPost {
  ...
  return {
    post: async (url, body): Promise<HttpPostResponse> => {
      ...
      // No status check. §3.4 L346 needs the 429 to reach `bot.ts` to honour
      // `parameters.retry_after`, and §4.2 L384 warns that a failure can just
