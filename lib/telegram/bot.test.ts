...
/**
 * An obvious placeholder. §7.6 L705 keeps the real token in Secrets Manager and
...
/** §4.2 L390 — a failure is HTTP 200 with `ok: false`, never a status code. */
...
describe("request shape (§4.2 L386)", () => {
  ...
  /** §4.2 L388 via `chatIdFor` — the chat id is the channel with a leading `@`. */
...
describe("the `ok` field is the error signal, not the HTTP status (§4.2 L390)", () => {
...
describe("pacing and retry (§3.4 L352)", () => {
  ...
  test("a 429 is retried exactly once, honouring retry_after in seconds", async () => {
    ...
    expect(response.ok).toBe(true);
    expect(response.result?.message_id).toBe(4242);
    // The retry wait replaces that send's pause: §3.4 L352 says "≥3 s", so a
  ...
  /** Telegram may wrap a rate limit in a 200 (§4.2 L390), so the envelope counts. */
...
describe("the token never escapes the request path (§7.6 L705)", () => {
...
describe("TelegramApiErrors (§7.7 L738)", () => {
