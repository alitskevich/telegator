...
/**
 * What the cookie carries. Deliberately not `enabled`: §8.6 L871 rejects a
...
/**
 * §8.6 L840 puts an operator console behind Cognito. `httpOnly` because an XSS
...
/**
 * The three gates of §8.4 L819 ("every action ... re-checks the caller's role
 * server-side") and §8.6 L871, in order: authenticated, not disabled, ranked at
 * or above `min`.
 *
 ...
 *
 * §8.6 L873: no branch of this function reads `process.env`, so there is no
