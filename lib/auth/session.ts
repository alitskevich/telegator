...
/**
 * What the cookie carries. Deliberately not `enabled`: §8.6 L875 rejects a
...
/**
 * §8.6 L844 puts an operator console behind Cognito. `httpOnly` because an XSS
...
/**
 * The three gates of §8.4 L823 ("every action ... re-checks the caller's role
 * server-side") and §8.6 L875, in order: authenticated, not disabled, ranked at
 * or above `min`.
 *
 ...
 *
 * §8.6 L877: no branch of this function reads `process.env`, so there is no
