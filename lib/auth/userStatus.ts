 ...
 *
 * §8.6 L875 — "a disabled user is rejected at every action" — needs a live
    ...
    async isEnabled(sub) {
      try {
        const user = await client.send(
          ...
          // created by an administrator, which §8.6 L875 requires them to be.
          new AdminGetUserCommand({ UserPoolId: config.userPoolId, Username: sub }),
        );
        return user.Enabled === true;
      } catch {
        /**
         * Every uncertain answer is "no". §8.6 L875 makes `enabled` the
