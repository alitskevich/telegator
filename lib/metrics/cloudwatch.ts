...
/**
 * The CloudWatch adapter for `MetricSink` (§7.7 L726–739).
 *
 * §7.7 L724 is why this is more than plumbing: "Because there is no items
...
/**
 * The only dimension keys §7.7 uses. §7.7 L741 refuses a per-category metric —
    ...
    count(name: MetricName, value: number, dimensions: MetricDimensions = {}): void {
      for (const key of Object.keys(dimensions)) {
        if (!ALLOWED_DIMENSIONS.has(key)) {
          // Dropped rather than emitted: §7.7 L741's whole point is that an
      ...
      // §8.5 L829–831 reads these with the `Sum` statistic, so N identical data
    ...
    async flush(): Promise<void> {
      ...
      for (let start = 0; start < entries.length; start += MAX_METRIC_DATA_PER_PUT) {
        ...
        try {
          ...
          // §1.3 L69 — a lost metric is bad; a stage killed by its own telemetry
 ...
 * counted, and §7.7 L724 makes those counts the system of record for volume.
