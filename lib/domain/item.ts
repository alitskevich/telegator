...
/**
 * The in-flight item — an SQS payload, never a table row (§2.2 L123, §1.3 L62).
 *
 * It exists in two shapes: Stage A leaves `scrape` for the analyze queue
 * (§2.2 L128–138), and Stage B leaves `analyze` for the aggregate queue
 * (§2.2 L140), adding the AI fields of §5.2 L443–455.
 */

/** §11.2 L1009 and §5.2 L448 — the source prompt's 60-symbol cap, raised to 220. */
export const SUMMARY_MAX_LENGTH = 220;

/** §2.2 L138 — replaces the source system's initial `status` value. */
export const ITEM_KINDS = ["post", "forward", "empty"] as const;

export const ItemKindSchema = z.enum(ITEM_KINDS);
export type ItemKind = z.infer<typeof ItemKindSchema>;

/** §2.2 L131 — resolves a `[text](#N)` token produced by §3.1 L213. */
...
export const ScrapedItemSchema = z.object({
  id: ItemIdSchema,
  /** Plain text with inline links replaced by `[text](#N)` tokens (§2.2 L130). */
  body: z.string(),
  links: z.array(LinkSchema).default([]),
  image: z.string().optional(),
  forwardedFrom: z.string().optional(),
  /** Copied from the source; §3.3 L284 falls back to `telegator_news` when absent. */
  tgChannel: z.string().optional(),
  date: DateKeySchema,
  /** The source default, overwritten by AI (§2.2 L136) — an open string, since an
...
/**
 * The AI fields, declared once. §5.2 L443 fixes which are required.
 *
 ...
 */
const aiField = {
  /** Essential subject in three words, English (§5.2 L447). */
  title: z.string(),
  /** Brief factual matter, in Belarusian, with `[text](#N)` tokens intact (§5.2 L448). */
  summary: z.string().max(SUMMARY_MAX_LENGTH),
  /** ISO-3166 alpha-2 (§5.2 L449). Case is normalised by §3.2 L254, not here. */
  country: z.string(),
  location: z.string(),
  category: z.string(),
...
/** The model's response shape (§5.2 L443–455). */
export const AiFieldsSchema = z.object(aiField);

export type AiFields = z.infer<typeof AiFieldsSchema>;

/**
 * Stage B — everything from Stage A plus the AI fields (§2.2 L140).
 *
 ...
 * keeps Stage A's meaning. §3.2 L254 merges tags rather than replacing them.
 */
export const AnalyzedItemSchema = ScrapedItemSchema.extend({
   ...
   * unfalsifiable at the stage that is supposed to satisfy it (§3.2 L254).
   */
