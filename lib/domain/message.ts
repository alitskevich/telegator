...
/** §3.4 L323 — publish renders the first 12 members, though 20 are stored. */
...
export const MemberBlockSchema = z.object({
  ...
  /** Source channel segment, for the `@mention` §3.4 L326 renders. */
  channel: z.string(),
  /** When this member joined — §3.4 L323 sorts by it for stable ordering. */
 ...
 *
 * R7: §6 L585/L584 build the record as `{...item, …}`, which would also write
 ...
 * descriptive fields overwrite" (§3.3 L287); this table is the schema, and
...
const messageFields = z.object({
  ...
  /**
   * R43/R44 — the match key §7.2 L640 projects, in place of the embedding it
  ...
  /**
   * Last-write epoch ms, and the sort key on **both** GSIs (§2.3 L163, §7.2 L638).
  ...
  /** R16 — §8.4 L814 mandates a soft delete; §2.3's table never declares it. */
 ...
 * A stored message.
 *
 * The refinement enforces §2.3 L155's cache invariant. §3.3 L283 recomputes
...
/**
 * The `status-index` projection (§7.2 L640, R27, amended by R44/R51).
 *
 * §7.2 L640 projects no `members`. Giving the projection its own type means
 * dashboard code cannot read an attribute the query did not return; §8.3 L802's
...
/**
 * The `date-index` projection (§7.2 L638/L636, R27, amended by R44/R51).
 *
 ...
 *
 * §6 L564's Pass 2 reads its candidates from this index. Typing them without
 * `members` is what stops R9's defect at the type level: §3.3 L283/L282 read
 ...
 *
 * `tgId` and `tgAt` are deliberately absent: §3.3 L288 preserves `tgId` so the
...
export const MessageMergeAttributesSchema = messageFields
  ...
  .partial()
  /**
   * `status` is optional (R39). §6 L585's merge branch normally returns the
   * message to `topublish` so §3.4 L349 edits the live post with the new
