...
/**
 * `messages` — aggregated, publishable stories (§2.3 L144–161).
 *
...
/** §2.3 L158 — the target channel a message falls back to. */
export const DEFAULT_TG_CHANNEL = "telegator_news";

/** §3.4 L317 — publish renders the first 12 members, though 20 are stored. */
export const MEMBER_RENDER_LIMIT = 12;

/** §2.3 L151. `error` currently has no writer — see R17. */
...
/**
 * §2.3 L166–173 — everything `publish` needs to render one item, captured at
 * aggregation time.
 *
...
export const MemberBlockSchema = z.object({
  /** Belarusian summary, with `[text](#N)` tokens still intact (§2.3 L168). */
  summary: z.string().max(SUMMARY_MAX_LENGTH),
  links: z.array(LinkSchema).default([]),
  /** Source channel segment, for the `@mention` §3.4 L320 renders. */
  channel: z.string(),
  /** When this member joined — §3.4 L317 sorts by it for stable ordering. */
...
/**
 * The field list of §2.3 L148–161, closed.
 *
 * R7: §6 L579/L582 build the record as `{...item, …}`, which would also write
 ...
 * descriptive fields overwrite" (§3.3 L281); this table is the schema, and
 * unlisted keys are stripped.
 */
const messageFields = z.object({
  /** Id of the **first** item that created the message (§2.3 L150). */
  id: ItemIdSchema,
  status: MessageStatusSchema,
  members: z.record(ItemIdSchema, MemberBlockSchema),
  /** Cached `size(members)`, so the dashboard need not read the map (§2.3 L153). */
  memberCount: z.number().int().nonnegative(),
  /**
   * R43/R44 — the match key §7.2 L634 projects, in place of the embedding it
  ...
  // Copied or merged from member items (§2.3 L157).
  ...
  /** Telegram `message_id`. Its presence turns the next publish into an edit (§2.3 L159). */
  tgId: z.string().optional(),
  tgAt: z.number().int().nonnegative().optional(),

  /**
   * Last-write epoch ms, and the sort key on **both** GSIs (§2.3 L161, §7.2 L632).
  ...
  /** R16 — §8.4 L799 mandates a soft delete; §2.3's table never declares it. */
 ...
 * A stored message.
 *
 * The refinement enforces §2.3 L153's cache invariant. §3.3 L277 recomputes
...
export const MessageSchema = messageFields.refine(
  ...
  { message: "memberCount must equal the size of members (§2.3 L153)", path: ["memberCount"] },
);

export type Message = z.infer<typeof MessageSchema>;

/**
 * The `status-index` projection (§7.2 L634, R27, amended by R44/R51).
 *
 * §7.2 L634 projects no `members`. Giving the projection its own type means
 * dashboard code cannot read an attribute the query did not return; §8.3 L787's
...
/**
 * The `date-index` projection (§7.2 L632/L634, R27, amended by R44/R51).
 *
 ...
 *
 * §6 L558's Pass 2 reads its candidates from this index. Typing them without
 * `members` is what stops R9's defect at the type level: §3.3 L277/L280 read
 ...
 *
 * `tgId` and `tgAt` are deliberately absent: §3.3 L282 preserves `tgId` so the
 * next publish is an edit (§2.3 L159), and R7 notes the §6 spread would
...
export const MessageMergeAttributesSchema = messageFields
  ...
  .partial()
  /**
   * `status` is optional (R39). §6 L579's merge branch normally returns the
   * message to `topublish` so §3.4 L343 edits the live post with the new
   ...
   * text. `memberCount` and `ts` stay required: §2.3 L153's invariant and §6
   * L591's stamp hold for every write.
   */
