...
/**
 * The source segment must not itself contain a slash: §6 L597 takes the channel
 * as `id.split("/")[0]`, which would silently truncate otherwise, and §3.4 L326
...
/** §3.1 L225 — `id = "{sourceId}/{messageId}"`. */
...
/** §6 L597's `item.id.split("/")[0]`, but strict: a malformed id throws rather
