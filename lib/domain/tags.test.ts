...
describe("mergeTags", () => {
  /** AC-2.3: source tags survive alongside AI tags, with no duplicates. */
  ...
  /**
   * §3.3 L286 calls `mergeTags(item.tags, match.tags)` — item first. First-seen
   ...
   * for case folding (§3.2 L260 says only "comma-split, deduplicated,
   * comma-joined"), and folding would discard a tag's stored form. §3.4 L342
