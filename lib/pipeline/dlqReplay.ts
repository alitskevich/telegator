...
/**
 * §3.5 L366–370 — the DLQ replay handler.
 *
...
export interface DlqReplayOptions {
  /** §8.4 L821 — the operator bounds the drain. */
  readonly max: number;
}
 ...
 *
 * The **group** is preserved: §3.3 L276 relies on it to serialise one date's
 ...
 * worst outcome for a recovery path. §3.5 L370 makes a new id safe: a duplicate
...
export async function replayDlq(
  ...
  // §3.5 L368's "replay counter" — the operator's receipt that the drain ran.
