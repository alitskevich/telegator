 ...
 * loop's rule is that real Telegram HTML is captured (§4.1 L374 makes the four
...
function source(fields: Record<string, unknown>): Source {
  return SourceSchema.parse({
    ...
    tgChannel: "target",
    // Long overdue, so selection (§3.1 L200) never gets in the way of what a
...
describe("runScrape — acceptance criteria", () => {
  test("AC-1.4 (§3.1 L233) an unreachable source increments zeroYieldRuns and leaves other sources unaffected", async () => {
    const h = harness({
      sources: [source({ id: CHANNEL }), source({ id: OTHER_CHANNEL })],
      // Only the second channel answers; the first is modelled the way §3.1 L205
  ...
  test("AC-1.4 (§3.1 L233) a fetcher that throws still leaves other sources unaffected", async () => {
    const h = harness({
      sources: [source({ id: CHANNEL }), source({ id: OTHER_CHANNEL })],
  ...
  test("AC-1.5 (§3.1 L234) a failed SendMessageBatch leaves lastItemId unchanged", async () => {
    const h = harness({
    ...
    // §3.1 L226 — "Cursor fields are written **only after** the enqueue
  ...
  test("AC-1.6 (§3.1 L235) a forwarded post is counted and dropped, never enqueued", async () => {
    const h = harness({
      sources: [source({})],
...
describe("runScrape — fetch", () => {
  test("§3.1 L205 the ?after= cursor is appended only when lastItemId exists", async () => {
  ...
  test("§3.1 L205 browser-like headers are sent on every request", async () => {
    const h = harness({
      sources: [source({})],
...
describe("runScrape — cursor", () => {
  ...
  test("§3.1 L218 a successful parse resets zeroYieldRuns and records lastNonZeroCount (R15)", async () => {
    const h = harness({
  ...
  test("R15 a zero-yield run zeroes lastCount but preserves lastNonZeroCount", async () => {
    ...
    // Without this, §4.1 L376's "non-zero historical lastCount" is gone two runs
...
describe("runScrape — metrics", () => {
  test("§7.7 L723 ItemsScraped is counted per source", async () => {
    const h = harness({
      sources: [source({ id: CHANNEL }), source({ id: OTHER_CHANNEL })],
      pages: {
  ...
  test("§4.1 L376 three consecutive zero-yield runs emit SourceStale, dimensioned and undimensioned (R25)", async () => {
    const repo = fakeSourceRepo([source({ lastCount: 0, lastNonZeroCount: 5 })]);
    const metrics = recordingMetrics();

    for (let run = 0; run < STALE_ZERO_YIELD_RUNS; run++) {
      // Each run needs the cold source to be due again (§3.1 L200).
    ...
    // R25 — §7.7 L739 alarms on "SourceStale for any source", and a CloudWatch
    // alarm cannot enumerate a runtime-discovered dimension at synth time.
    expect(metrics.get("SourceStale", {})).toBe(1);
  });

  test("§4.1 L376 a source that never yielded anything does not alarm", async () => {
...
describe("runScrape — enqueue", () => {
  test("§3.1 L224 posts are sent in batches of SQS_MAX_BATCH_ENTRIES", async () => {
  ...
  test("§3.1 L224 only kind === 'post' items reach the analyze queue", async () => {
    const h = harness({
      sources: [source({})],
    ...
    // §2.2 L135 — the Standard analyze queue carries no group id.
    expect(h.queue.sent[0]?.messageGroupId).toBeUndefined();
  });

  test("§2.2 L135 every post of one run shares a single date key", async () => {
    const h = harness({
      sources: [source({ id: CHANNEL }), source({ id: OTHER_CHANNEL })],
      pages: {
  ...
  test("§3.1 L197 only sources selectable by §3.1 L200 are polled", async () => {
    const h = harness({
      sources: [
