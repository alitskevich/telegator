...
/**
 * §3.1 L222 — the per-post transform: a parsed post plus its source become one
 * Stage A item payload (§2.2 L128–138).
 *
...
/**
 * The parse output of §3.1 L211–218, declared structurally rather than imported
...
export interface TransformInput {
  /** The Telegram message id alone — digits (§3.1 L211), not the composite id. */
  id: string;
  /** Already tokenised: inline links are `[text](#N)` (§3.1 L213). */
  body: string;
  links: Link[];
  image?: string;
 ...
 * (§2.1 L114).
 *
 * §3.1 L222 says only "strip the source's `teaser` from the body" and leaves
 ...
 *    separator whitespace §3.1 L215 had collapsed behind it, and that whitespace
 ...
 * touch.
 *
 * Note this runs on the **already tokenised** body (§3.1 L213), so a teaser
 ...
 * running before tokenisation, which L222 places after parsing.
 */
...
/**
 * §3.1 L222's classification. The order is normative: `forward` is tested
 * before `empty`, so a forwarded post with a blank body is a `forward`. Both
 * are dropped by §3.1 L224, but the counter metric there distinguishes them.
 *
 ...
 *
 * Blankness is judged on the *stripped* body, following L222's own order: a post
 ...
 * so every post of a run shares one date key. That matters because §2.2 L135
...
export function transformPost(post: TransformInput, source: Source, date: DateKey): ScrapedItem {
  ...
  return ScrapedItemSchema.parse({
    ...
    // Stamped from the source; §2.2 L136 lets the analyze stage overwrite
