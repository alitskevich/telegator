...
/**
 * §3.4 L322–345 — message assembly and the send-mode decision, as pure
 * functions.
 *
 ...
 * previously published posts (§3.4 L343) and is replayed by SQS after a failure,
 ...
 * republish rewrites a post that did not change (AC-4.6, L357).
 */

/**
 * **R13 — the photo threshold is 1012, not §3.4 L342's 1024 caption limit.**
 ...
 * (§4.2 L385), and collapsing the two would lose the 12 characters R13 turns on.
 */
 ...
 * §3.4 L348) instead of succeeding as an empty post.
 */
...
/** The §3.4 L326 blank line, reused to set the hashtag line off from the members. */
const BLANK_LINE = "\n\n";

/** §3.4 L334 — "joined with `\", \"`". */
const HEADER_PART_SEPARATOR = ", ";

/** Absent, empty and whitespace-only are all "empty" for §3.4 L334 and L345. */
function hasValue(value: string | undefined): value is string {
  return value !== undefined && value.trim() !== "";
}

/** The fields §3.4 L325/L334 draw the header from. */
export type HeaderSource = Pick<Message, "date" | "country" | "location" | "category">;

/**
 * §3.4 L325 — `<b>⚡️</b> <i>{date}</i> <b>{COUNTRY, location, category}</b>`,
 * the parts being the non-empty `country` (uppercased), `location` and
 * `category` in that order (L334). Exported so the format can be pinned alone.
 *
 * `date` is not escaped: `DateKeySchema` (§2.3 L157) admits only digits and
 * hyphens.
 *
...
/**
 * §3.4 L325–332's layout, plus R12's hashtag line — after the member blocks,
...
/**
 * **Recorded decision, not spec text.** §3.4 L338 caps a message at 4096
 * characters and the spec states no truncation rule — because before R12 the
 * cap was unreachable: 12 blocks (L317) of a 220-character summary (§11.2)
...
function fitToLimit(header: string, blocks: readonly string[], hashtagLine: string): string {
  ...
  // Nothing fits. Emit the floor and let the Bot API reject it (§3.4 L348).
  return compose(header, blocks.slice(0, MIN_RENDERED_MEMBERS), "");
}

/** §4.2 L380 — the three methods, and only these three. */
export type SendMethod = "sendMessage" | "sendPhoto" | "editMessageText";

export interface AssembledMessage {
  readonly text: string;
  readonly method: SendMethod;
  /** Present only on `sendPhoto`; §3.4 L343 never re-sends a photo on an edit. */
...
/**
 * §3.4 L322–345 — the whole publish payload decision for one message.
 *
 * The stage that calls this owns the status check (L315), the pacing and the
 * retry (L346); this function owns only what to send.
 */
export function assembleMessage(message: Message): AssembledMessage {
  ...
  /** §3.4 L345 — "link preview disabled when the message has a title or image". */
  const disableWebPagePreview = hasValue(message.title) || hasValue(message.image);

  // §3.4 L343 — a `tgId` makes this an edit (AC-4.1, L352), and an edit never
  ...
  // §3.4 L342/L344 — a photo only when there is one and the text still fits
