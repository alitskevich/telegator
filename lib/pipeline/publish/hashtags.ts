...
/**
 * The §3.4 L342 hashtag line (R12, §11.3 L1046).
 *
...
/**
 * §3.4 L342: "every `title` word longer than 4 characters" — so 5 and up.
...
/**
 * §3.4 L342 drops `none`/`null`. The spec does not say whether that comparison
...
/** "`.,@!'"()` removed" — exactly the eight characters §3.4 L342 lists. */
const REMOVED_CHARACTERS = /[.,@!'"()]/g;

/** Title words are whitespace-delimited; a hyphen joins one word, it does not end it. */
const WHITESPACE = /\s+/;

/** The fields §3.4 L342 draws the line from, all optional but `date` and `ts`. */
...
/**
 * §3.4 L342's `#hashtag` form: spaces and hyphens to `_`, `.,@!'"()` removed,
...
/** §3.4 L342's "every `title` word longer than 4 characters". */
...
/** §3.4 L342's "`none`/`null`/empty dropped", case-folded per the note above. */
 ...
 *
 * Order is first-seen, over the sources in the order §3.4 L342 names them. That
 * determinism is required, not cosmetic: AC-3.7 makes replay
 * byte-identical, and §3.4 L348 re-sends a message by edit, so a line that
