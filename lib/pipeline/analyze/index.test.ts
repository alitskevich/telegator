 ...
 * actually have enqueued (§7.3 L650 carries `ScrapedItemSchema`).
 */
...
describe("runAnalyze — acceptance criteria", () => {
  test("AC-2.1 (§3.2 L266) an item classified importance: low never reaches the aggregate queue", async () => {
    ...
    // §3.2 L262 — a `skip` decision is final, so the message is NOT failed back
    // to SQS. Only a provider error takes the retry path.
    expect(result.batchItemFailures).toEqual([]);
  });

  test("AC-2.2 (§3.2 L267) a provider error on one message leaves the other nine successfully processed", async () => {
  ...
  test("AC-2.5 (§3.2 L270) a failed item is reported as a batch item failure with its payload untouched", async () => {
    ...
    // (§7.3 L650), configured in infra — no code in this stage can make it true
    ...
    // DLQ copy is replayable (§3.2 L262).
...
describe("runAnalyze — pre-filter (§3.2 L247)", () => {
...
describe("runAnalyze — enqueue to aggregate (§3.2 L258)", () => {
  /**
   * AC-3.9 (§3.3 L310), the half this code owns. The criterion — "two items with
  ...
  test("the enqueued payload is normalised — country uppercased, tags merged (§3.2 L260)", async () => {
  ...
  test("ItemsAnalyzed counts only the items that reached the aggregate queue (§7.7 L732)", async () => {
  ...
  test("a rejected SendMessageBatch entry fails that message rather than losing it", async () => {
    // `send` reports partial failures in its result and never throws (§3.1 L231
...
describe("runAnalyze — category log line (§7.7 L741, §8.5 L835)", () => {
  test("the category field name is `category`, at the top level of the JSON line", () => {
    // §8.5 L835's chart is a Logs Insights `stats count(*) by category` over this
...
describe("buildClassificationRequest — §5.2 L426–435", () => {
  ...
  test("§5.2 L465 — neither temperature nor top_p is carried over", () => {
    const request = buildClassificationRequest("Prose to classify.");
