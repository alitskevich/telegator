 ...
 * actually have enqueued (§7.3 L646 carries `ScrapedItemSchema`).
 */
...
describe("runAnalyze — acceptance criteria", () => {
  test("AC-2.1 (§3.2 L262) an item classified importance: low never reaches the aggregate queue", async () => {
    ...
    // §3.2 L258 — a `skip` decision is final, so the message is NOT failed back
    // to SQS. Only a provider error takes the retry path.
    expect(result.batchItemFailures).toEqual([]);
  });

  test("AC-2.2 (§3.2 L263) a provider error on one message leaves the other nine successfully processed", async () => {
  ...
  test("AC-2.5 (§3.2 L266) a failed item is reported as a batch item failure with its payload untouched", async () => {
    ...
    // (§7.3 L646), configured in infra — no code in this stage can make it true
    ...
    // DLQ copy is replayable (§3.2 L258).
...
describe("runAnalyze — pre-filter (§3.2 L243)", () => {
...
describe("runAnalyze — enqueue to aggregate (§3.2 L254)", () => {
  /**
   * AC-3.9 (§3.3 L306), the half this code owns. The criterion — "two items with
  ...
  test("the enqueued payload is normalised — country uppercased, tags merged (§3.2 L256)", async () => {
  ...
  test("ItemsAnalyzed counts only the items that reached the aggregate queue (§7.7 L728)", async () => {
  ...
  test("a rejected SendMessageBatch entry fails that message rather than losing it", async () => {
    // `send` reports partial failures in its result and never throws (§3.1 L228
...
describe("runAnalyze — category log line (§7.7 L737, §8.5 L831)", () => {
  test("the category field name is `category`, at the top level of the JSON line", () => {
    // §8.5 L831's chart is a Logs Insights `stats count(*) by category` over this
...
describe("buildClassificationRequest — §5.2 L422–431", () => {
  ...
  test("§5.2 L461 — neither temperature nor top_p is carried over", () => {
    const request = buildClassificationRequest("Prose to classify.");
