...
/**
 * Stage 2 — the `analyze` SQS consumer (§3.2 L239–258).
 *
 * The stage owns effects only. Every decision it takes comes from `route.ts`
 * (item 3.6): the pre-filter, the routing table and the §3.2 L256 normalisers
...
/** §7.3 L646 and §3.2 L241 — the event source mapping delivers at most ten records. */
...
/**
 * The `output_config.format` of §5.2 L427.
 *
...
/**
 * The §5.2 L422–431 request body.
 *
 * §5.2 L461: `temperature` and `top_p` are **not** carried over — they are
...
export interface ClassificationRequestOptions {
   ...
   *
   * §5.2 L425 sets `effort: "low"` and L461 makes it the replacement for the
...
/**
 * Builds the classification request of §5.2 L422–431.
 *
...
/**
 * Consumes one batch of analyze-queue records (§3.2 L239–258).
 *
 * Items are classified **sequentially**. §7.5 L689 gives the function 300 s for
...
export async function runAnalyze(
  ...
  for (const sqsRecord of records) {
    try {
      ...
      // §3.2 L243 — empty or bare-link bodies are dropped before the AI call.
      ...
      // §3.2 L247 — one request per item. A provider error propagates out of
      // `classify` (lib/ai/ports.ts) into the catch below, which is §3.2 L258's
      ...
      deps.logger.info(CLASSIFIED_LOG_MESSAGE, {
        ...
        // Carried so §8.5 L831's chart can be narrowed to enqueued items later
        ...
        // exactly the categories §5.2 L455 tells the model to diminish.
        decision: decision.kind,
      });

      if (decision.kind === "retry") {
        // §3.2 L249 — "No category returned" shares the provider-error row of
        ...
        // `crime&law` branch is: §5.2 L427 constrains the response to §5.4's
      ...
      if (decision.kind === "drop") {
        // §3.2 L258 — a `skip` decision is final. No throw, no failure id: the
      ...
      // §7.3 L664 — exactly this message fails. Without per-message reporting
 ...
 * (§7.7 L720) nothing would ever notice.
 */
async function sendCandidates(
  ...
  try {
    ...
    for (const index of result.successful) {
      ...
      // §7.7 L728 — `ItemsAnalyzed` counts items that actually reached the next
      // stage, so it stays disjoint from `ItemsSkipped` and the two add up to
      // what §8.5 L826–828's cards claim.
    ...
    // cost is the point of §7.3 L664's per-message reporting being applied to
