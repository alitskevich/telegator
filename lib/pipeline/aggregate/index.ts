 ...
 * separately tested; what is left here is the four steps §6 L590–591 leave to
 ...
 * stage hands it. Emitting any again would double-count a number §7.7 L724
 * makes the pipeline's system of record.
 */

export interface AggregateDeps {
  /**
   * R46 — §3.3 L278's single `embedBatch` call for the batch, replaced by at
  ...
  /** §7.3 L652 — the publish FIFO queue. */
  readonly queue: QueueProducer;
  readonly clock: Clock;
  readonly metrics: MetricSink;
  readonly logger: Logger;
  /**
   * §10.3 L1021 requires the decision boundary to be recalibrated before
 ...
 * typing it here keeps `aws-lambda` out of `lib/`, which §8.2 L792 reserves for
...
export async function runAggregate(
  ...
  try {
    ...
    // the whole batch (§7.3 L668). An adjudication failure is NOT one of these —
    // §10.3 L1011 makes it split rather than throw, inside `dedupBatch`.
  ...
  // §6 L590–591 — write first, enqueue second, and only what was written. An id
...
/**
 * §3.3 L292–296 — one publish enqueue per touched id.
 *
 * `publishQueueMessage` sets `MessageGroupId` and `MessageDeduplicationId` to the
 * message id (L294–295). **R19: no per-message `DelaySeconds`.** SQS FIFO
 * supports only a queue-level delay, so L292's settle delay is configured on the
 * publish queue itself (§7.3 L652) and cannot be set here.
 */
async function enqueuePublish(
  ...
  for (const chunk of chunks(messageIds, SQS_MAX_BATCH_ENTRIES)) {
    ...
    // replays the item, and §6 L597's `members`-keyed map makes that a no-op
...
/**
 * §7.3 L668 — a body that is not a valid Stage B payload is one item's failure,
