...
/** A second date, which §6 L550's correctness rule keeps entirely separate. */
...
/** An SQS record as the event source mapping delivers it (§7.3 L668). */
...
describe("§3.3 aggregate consumer", () => {
  /**
   * R47 — §3.3 L302 reads "two items at cosine similarity 0.90 with the same
  ...
  test("denormalization (§1.3 L68, §2.3): each member block carries summary, links, channel and ts", async () => {
    ...
    const message = await readBack(h.repo, "chan_a/1");
    // Everything §3.4 L323-326 renders, present without a second read.
  ...
  /**
   * R51 — the wording is unchanged, but not the mechanism. Under §3.3 L287 a
  ...
  test("AC-3.7: replaying the identical batch leaves members, memberCount and tags unchanged", async () => {
    ...
    /**
     * R45 — §6 L597 conceded the embedding drifts on every replay, "bounded and
  ...
  test("AC-3.8: a 21st member is rejected and memberCount stays at 20", async () => {
    ...
    // Dropped outright (§3.3 L283): no message of its own, and nothing to publish.
  ...
  test("every touched id is enqueued with group and dedup ids equal to the message id (L290-291)", async () => {
    ...
    /**
     * AC-4.6 (§3.4 L363) — the STAGE's enqueue, not just the builder's.
     ...
     * concurrently.
     *
     * Different dates never merge (§6 L550), so both ids are touched.
     */
  ...
  test("a malformed record body is one batch item failure, not a thrown batch (§7.3 L668)", async () => {
  ...
  /** §10.3 L1021, as rewritten by R48 — two thresholds now, still injected. */
  test("§10.3 L1021: the band is configurable, not compiled in", async () => {
    const a = item("chan_a/1", SAME_EVENT);
    const b = item("chan_b/2", SAME_EVENT);
    const records = [sqsRecord(a), sqsRecord(b)];
