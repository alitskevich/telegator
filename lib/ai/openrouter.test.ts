...
describe("createOpenRouterClassifier", () => {
  ...
  it("throws when the response violates NewsItemSchema (§3.2 L251)", async () => {
    const fake = fakeClassifierClient(async () =>
  ...
  it("propagates a provider error unchanged (§3.2 L258)", async () => {
   ...
   * name itself. §3.2 L258 sends a throw to SQS retry, so an unnamed one would
