...
describe("NewsItemSchema", () => {
  ...
  /**
   * Narrower than the payload schema of item 2.5: §5.2 L453 constrains the
   * model's category to §5.4's enum, while §2.2 L138 lets the item carry an
  ...
  test("keeps §5.2 L445's required/optional split", () => {
...
describe("NEWS_ITEM_SCHEMA (the JSON Schema §5.2 L427 sends)", () => {
  test("declares exactly the six required fields of §5.2 L445", () => {
  ...
  test("enumerates importance as high or low (§5.2 L454)", () => {
    expect(propertyOf("importance").enum).toEqual(["high", "low"]);
  });

  /**
   * §5.2 L447-457's Description column is the only place the "three words",
 ...
 * L251's transient path into the DLQ.
 */
