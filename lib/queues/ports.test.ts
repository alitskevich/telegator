...
describe("analyzeQueueMessage", () => {
  ...
  /** §7.3 L650 — telegator-analyze is a Standard queue; group and dedup ids are FIFO-only. */
...
describe("aggregateQueueMessage", () => {
  /**
   * §3.2 L258 and §7.3 L651. The group is the date because §3.3 L276 uses it to
   ...
   */
  /**
   * AC-3.9 (§3.3 L310) — "Two items with the same `date` are never processed by
...
describe("publishQueueMessage", () => {
  /**
   * §3.3 L294-295 — the group serialises edits to one Telegram message and the
   * deduplication id collapses repeats inside SQS's window.
   *
   * AC-4.6 (§3.4 L363) rests on both. The window itself is SQS's and is BLOCKED;
   ...
   * per-message settle delay is set on the queue (§7.3 L652) and never here.
   */
...
describe("fakeQueueProducer", () => {
  ...
  /**
   * The behaviour AC-1.5 rests on. A real SendMessageBatch returns HTTP
...
describe("SQS_MAX_BATCH_ENTRIES", () => {
  /** §3.1 L227 — "via SendMessageBatch (10 per call)", which is the SQS API limit. */
