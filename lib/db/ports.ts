 ...
 *
 * DynamoDB is unreachable from the build machine and §10.1 L990's DynamoDB
 ...
 * patterns §7.2 L637–638 indexes for, not by a generic CRUD surface: a repo
...
export interface SourceRepo {
  ...
  /**
   * §3.1 L231 — the cursor write that happens only after the enqueue succeeds.
  ...
  /**
   * §8.3 L801 and §8.4 L816 — every source, whatever its status.
   *
  ...
  /** §8.4 L812 — an operator edit, attribute-level. The caller validates the delta. */
  patch(id: string, delta: Readonly<Record<string, unknown>>): Promise<void>;
  /** §8.4 L814 — soft delete. The row survives; R16 hides it from reads. */
...
export interface TargetRepo {
  ...
  /** §8.3 L801's table, as for sources: a Scan, soft-deleted rows filtered. */
  listAll(): Promise<Target[]>;
  /** §8.4 L812 — an operator create. */
  ...
  /** §8.4 L814 — soft delete. The row survives; `listAll` hides it. */
  softDelete(ids: readonly string[]): Promise<void>;
}
 ...
 * several items into the same message (§6 L585 keys `pending` by message id).
...
export interface MessageRepo {
  /** §3.4 L321 and R9 — a base-table read, the only access that returns `members`. */
  get(id: string): Promise<Message | undefined>;
  /** §6 L564 — `date-index`, projecting the match key (R44) and nothing else that matters. */
  ...
  /**
   * §8.5 L832 — the "Messages published" card, window "all".
   *
  ...
  /** §6 L588's create branch — a whole new record. */
  putNew(message: Message): Promise<void>;
  /**
   * §6 L585's merge branch, written attribute-level.
   *
   * This is R9's resolution. §6 L590 says "WRITE pending.values()", which reads
