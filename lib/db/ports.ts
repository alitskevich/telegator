 ...
 *
 * DynamoDB is unreachable from the build machine and §10.1 L954's DynamoDB
 ...
 * patterns §7.2 L631–632 indexes for, not by a generic CRUD surface: a repo
 * that offered `scan` would invite §1.3 L67's forbidden table scan.
 */

export interface SourceRepo {
  get(id: string): Promise<Source | undefined>;
  /** §3.1 L197 — `status-index`. Excludes soft-deleted sources (R16). */
  listByStatus(status: string): Promise<Source[]>;
  put(source: Source): Promise<void>;
  /**
   * §3.1 L226 — the cursor write that happens only after the enqueue succeeds.
  ...
  /**
   * §8.3 L786 and §8.4 L801 — every source, whatever its status.
   *
  ...
  /** §8.4 L797 — an operator edit, attribute-level. The caller validates the delta. */
  patch(id: string, delta: Readonly<Record<string, unknown>>): Promise<void>;
  /** §8.4 L799 — soft delete. The row survives; R16 hides it from reads. */
 ...
 * several items into the same message (§6 L579 keys `pending` by message id).
 * Emitting one write per member instead would publish an intermediate
 * `memberCount` that disagrees with the map — a state §2.3 L153's invariant
...
/** §3.4 L348 — the result write after a successful send or edit. */
...
export interface MessageRepo {
  /** §3.4 L315 and R9 — a base-table read, the only access that returns `members`. */
  get(id: string): Promise<Message | undefined>;
  /** §6 L558 — `date-index`, projecting the match key (R44) and nothing else that matters. */
  queryByDate(date: string): Promise<DedupCandidate[]>;
  /** §8.5 L821 — `status-index`, `ts` descending. */
  queryByStatus(status: MessageStatus, limit?: number): Promise<MessageListItem[]>;
  /**
   * §8.5 L817 — the "Messages published" card, window "all".
   *
  ...
  /** §6 L582's create branch — a whole new record. */
  putNew(message: Message): Promise<void>;
  /**
   * §6 L579's merge branch, written attribute-level.
   *
   * This is R9's resolution. §6 L584 says "WRITE pending.values()", which reads
   * as a whole-record put; §2.3 L178 describes the same write as "writes
