...
/**
 * §8.4 L818's "Scrape now", as a command.
 *
 * It **invokes the deployed function**; it does not run `lib/pipeline/scrape/`
 * here. That is §8.2 L792's rule, and the reason it gives applies to a terminal
 ...
 *
 * No `--execute` gate, unlike `deploy`. A scrape is what §7.5 L693's schedule
 ...
 * does advance §3.1 L231's cursors, so it is not free — but it is the ordinary
 * operation of the pipeline, not a change to it.
 */

/** §7.5 L693's function, named the way every other resource is (§9.2 L900). */
...
async function main(): Promise<void> {
  ...
  // §3.1 L231 advances a cursor per source, so a second run right after a first
