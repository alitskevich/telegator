 ...
 * is the sole duplicate-suppression mechanism (§3.1 L223) its whole newest
 ...
 * which is correct for a cutover (`reseed-cursors.ts`, §9.5 L979) and wrong
...
async function main(): Promise<void> {
  ...
  for (const source of sources) {
    ...
    if (newest === undefined) {
      ...
      // does, so this is reported, never acted on. §4.1 L382's `SourceStale`
  ...
  if (write) {
    for (const advance of advances) {
      // A patch, not a put: §3.1 L231's `updateCursor` writes only the cursor,
